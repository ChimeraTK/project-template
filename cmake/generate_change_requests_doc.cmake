...
# configuration step, so it does not need to be maintained by hand. The page is only generated when at least one
# change request document exists. The generated page is written to the build directory and added to the Doxygen
# INPUT through the variable DOXYGEN_EXTRA_INPUT (see cmake/Doxyfile.in).
#
...
# are linked from the generated page with \subpage commands using these page names, so they appear nested under the
# 'Change Requests' page in the navigation tree instead of cluttering the top level.
#
#######################################################################################################################
...
foreach(change_request IN LISTS change_request_documents)
  ...
  string(APPEND CR_DOXYGEN_LIST "\\li \\subpage md_doc_2change-requests_2${change_request_page_name}\n")
endforeach()

# Only generate the 'Change Requests' page when at least one change request document exists. Otherwise the page
# would be empty and is not added to the Doxygen INPUT either.
if(change_request_documents)
  ...
  # no change request documents present, do not add anything to the Doxygen INPUT
